...
const Cell: React.FC<{ value: boolean | string }> = ({ value }) => {
  if (typeof value === 'string') return <span className="text-white text-xs font-bold">{value}</span>;
  return value
    ? <span className="text-green-400 text-base">✓</span>
    : <span className="text-slate-700 text-base">—</span>;
};

const FreePremium: React.FC = () => {
    ...
    <div className="flex flex-col min-h-full bg-[#07070F]">
      ...
      <header className="px-6 pt-6 pb-2 text-center relative">
        <button
          ...
          className="absolute left-6 top-6 w-9 h-9 rounded-xl bg-[#111118] border border-[#1E1E32] flex items-center justify-center text-slate-400 hover:text-white transition"
        >
          ←
        </button>
        <h1 className="text-white text-2xl font-black">Elige tu plan</h1>
        <p className="text-slate-500 text-xs mt-1">Desbloquea todo tu potencial</p>
      </header>
      ...
      <div className="flex-1 px-4 pb-6 overflow-y-auto">
        ...
        <div className="flex gap-3 mt-6 mb-6">
          ...
          <div className="flex-1 bg-[#111118] border border-[#1E1E32] rounded-2xl p-4 text-center">
            <p className="text-slate-400 text-[10px] font-black uppercase">Free</p>
            <p className="text-white text-2xl font-black mt-1">$0</p>
            <p className="text-slate-600 text-[10px]">para siempre</p>
          </div>
          {/* Pro */}
          <div className="flex-1 bg-green-600/10 border border-green-600/50 rounded-2xl p-4 text-center relative">
            <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-green-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase">Popular</div>
            <p className="text-green-400 text-[10px] font-black uppercase">Pro</p>
            <p className="text-white text-2xl font-black mt-1">$19</p>
            <p className="text-slate-500 text-[10px]">/ mes</p>
          </div>
          {/* Elite */}
          <div className="flex-1 bg-amber-500/10 border border-amber-500/40 rounded-2xl p-4 text-center">
            ...
            <p className="text-white text-2xl font-black mt-1">$49</p>
            <p className="text-slate-500 text-[10px]">/ mes</p>
          </div>
        </div>
        ...
        <div className="bg-[#111118] border border-[#1E1E32] rounded-2xl overflow-hidden mb-6">
          ...
          <div className="grid grid-cols-4 border-b border-[#1E1E32] px-4 py-2">
            <div />
            <div className="text-center text-slate-500 text-[10px] font-black uppercase">Free</div>
      ...
      <footer className="px-6 pb-8 space-y-3">
        ...
        <Button fullWidth variant="ghost" className="text-slate-500" onClick={() => navigate('/home')}>
          Continuar gratis
        </Button>
      </footer>
    </div>
