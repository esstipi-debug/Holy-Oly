"""
ETL Nutrition — Ingesta NUTRITION_BRAIN.md al RAG
...
def ingest_nutrition():
    print("Starting Nutrition Brain Ingestion...")
    vs = VectorStore()
    chunker = Chunker()

    source = ROOT_DIR / "nutrition" / "NUTRITION_BRAIN.md"
    ...
    for i, chunk_text in enumerate(chunks):
        ...
        chunk_uuid = chunker.generate_deterministic_id("nutrition", source.name, i)
        metadata = chunker.generate_metadata(
            brand="peak_qual",
            sport="shared",
            topic="nutrition_protocol",
            source=source.name,
            section="header",
            text=chunk_text
        )
    ...
    print(f"Nutrition Brain: {total} chunks ingested")
    return total
